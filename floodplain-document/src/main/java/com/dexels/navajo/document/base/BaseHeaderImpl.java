...
public class BaseHeaderImpl extends BaseNode implements Header {

  private static final Logger logger = LoggerFactory.getLogger(BaseHeaderImpl.class);
  ...
  public BaseHeaderImpl(com.dexels.navajo.document.Navajo n, String user, String password, String service) {
    super(n);
  ...
  /**
   * Returns whether the asynchronous server process has completed Add object ref
   * for supporting multiple async objects!
   */
  @Override
  ...
  public boolean hasCallBackPointers() {
	  return (getCallback() != null && !getCallback().getChildren().isEmpty());
  }
