...
//	private ReplicationMessage playerMessage;
	...
	public void setUp() throws Exception {
		...
		try(InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("address2.json")) {
			differentAddressMessage = ReplicationFactory.getInstance().parseStream(resourceAsStream);
		}
		try(InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("address1_identical.json")) {
		...
		try(InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("calendarday.json")) {
...
//		} 
		try(InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("player2.json")) {
			playerMessage2 = ReplicationFactory.getInstance().parseStream(resourceAsStream);
		} 
		try(InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("multikeys.json")) {
			multikeys = ReplicationFactory.getInstance().parseStream(resourceAsStream);
		} 
        try (InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("testsfacilitylocation.json")) {
            coordinatesMessage = ReplicationFactory.getInstance().parseStream(resourceAsStream);
        }

		try(InputStream resourceAsStream = TestTransformations.class.getResourceAsStream("persontransform.xml")) {
	...
	public void testTopicNameConstruction() {
		...
		result = CoreOperators.topicName("@TOPICNAME#112",new TopologyContext(Optional.of("MYTENANT"), "deployment", "currentinstance", "111"));
		Assert.assertEquals("MYTENANT-deployment-112-currentinstance-TOPICNAME", result);
		...
		result = CoreOperators.topicName("@TOPICNAME#112",new TopologyContext(Optional.empty(), "deployment", "currentinstance", "111"));
		Assert.assertEquals("deployment-112-currentinstance-TOPICNAME", result);
