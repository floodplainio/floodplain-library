...
public class ElasticSink implements Sink {
	...
	public FlowableTransformer<ReplicationMessage, Completable> createTransformer(Map<String,String> attributes,
		...
		if(!streamConfiguration.isPresent()) {
			throw new StreamTopologyException("Failed configuration: ElasticSink needs config");
		}
		...
		return flow->flow.compose(ElasticInsertTransformer.elasticSearchInserter(indexExtractor, typeExtractor, 200))
