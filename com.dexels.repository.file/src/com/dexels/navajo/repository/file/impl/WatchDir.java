...
public class WatchDir implements Closeable,Runnable {
	...
	private void start() {
		monitorThread = new Thread(this,"WatchdirThread");
		monitorThread.start();
	...
	public void run() {
		...
		try {
			...
			logger.info("Chilling out for a moment an then I'll continue");
			try {
				Thread.sleep(20000);
		...
		logger.warn("Watch thread terminated");
	}
	/**
	...
	public WatchDir(BaseFileRepositoryInstanceImpl fileRepository,List<String> monitored) throws IOException {
		...
		for (String folder : monitored) {
			...
			logger.info("Registering folder: "+folderF.getAbsolutePath());
			if(folderF.exists()) {
