...
public class Dependency {
	...
	public File getFilePathForDependency(File base, String securityHeader) {
		...
		File securyFile = versionFile;
		if (securityHeader != null) {
		    securyFile = new File(securyFile, securityHeader);
		}
		File f = new File(securyFile,getFileNameWithVersion());
		return f;
	}
	
	public File getSimplePathForDependency(File base) {
		String group = groupId.replaceAll("\\.", "/");
		File groupFile  = new File(base,group);
		File articleFile = new File(groupFile,artifactId);
		File versionFile = new File(articleFile,version);
