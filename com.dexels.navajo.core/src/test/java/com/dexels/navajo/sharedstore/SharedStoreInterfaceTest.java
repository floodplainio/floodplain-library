...
//	}	
	...
	public void testStoreWithLock() throws Exception {
		...
		for (int i = 0; i < MAXTHREADS; i++) {
			SerializableObject o = (SerializableObject) si.get("myparent",
					"mystoredobject" + i);
	...
	public void testRemoveAll() throws Exception {
		si.store("myparent", "mystoredobject1", new SerializableObject(),
				false, false);
		si.store("myparent", "mystoredobject2", new SerializableObject(),
				false, false);
	...
	public void testLockStringStringStringIntBoolean2() throws Exception {
		SharedStoreLock ssl = si.lock("myparent", "mylockfile", "owner1",
				SharedFileStore.READ_WRITE_LOCK, false);
	...
	@Test
	public void testLockStringStringStringIntBooleanWithMultipleThreads()
			throws Exception {
	...
	@Test
	public void testLockStringStringStringIntBooleanWithMultipleThreadsAndWaits()
			throws Exception {
	...
	public void testLockStringStringIntBoolean() {
		String instance = DispatcherFactory.getInstance().getNavajoConfig()
				.getInstanceName();
		System.err.println("instance = " + instance);
		SharedStoreLock ssl = si.lock("myparent", "mylock",
				SharedFileStore.READ_WRITE_LOCK, false);
	...
	public void testSetLastModifiedUnknownObject() throws Exception {
		...
		try {
			si.setLastModified("myparet", "myobject",
					Long.parseLong("1222180873000"));
		} catch (Exception e) {
			exception = true;
		}
	...
	public void testGetStream() throws Exception {
		si.store("myparent", "myobject", new SerializableObject(), false, false);
		ObjectInputStream ois = new ObjectInputStream(si.getStream("myparent",
				"myobject"));
	...
	public void testGetOutputStream() throws Exception {
		...
		// Write plain text.
		OutputStreamWriter os2 = new OutputStreamWriter(si.getOutputStream(
				"myparent", "mytext", false));
		String s = "some text";
		os2.write(s);
		os2.close();
		java.io.BufferedReader is = new BufferedReader(
	...
	public void testGetParentObjects() throws Exception {
		...
		SerializableObject s1 = new SerializableObject();
		s1.setField("ONE");
		SerializableObject s2 = new SerializableObject();
		s2.setField("TWO");
		SerializableObject s3 = new SerializableObject();
		s3.setField("THREE");
	...
	public void testGetObjects() throws Exception {
		...
		for (int i = 0; i < obs.length; i++) {
			...
			SerializableObject so = (SerializableObject) si.get("myparent",
					obs[i]);
